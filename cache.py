...
def get_combined_returns(_db: DatabaseInterface, start_date: date, end_date: date, fund: str = "DADCO") -> pd.DataFrame:
    """
    Fetch and combine portfolio and benchmark returns.
    This replaces the old get_daily_returns function.
    """
...
@st.cache_data(ttl=CacheConfig.DB_CACHE_TTL)
def get_balances(_db: DatabaseInterface, fund: str = "DADCO", start_date: str = None,
...
@st.cache_data(ttl=CacheConfig.DB_CACHE_TTL)
def get_cash_balance(_db: DatabaseInterface, fund: str = "DADCO", start_date: date = None,
...
@st.cache_data(ttl=CacheConfig.PROCESSED_CACHE_TTL)
def calculate_equity_returns(stock_daily_returns: pd.DataFrame, holdings: pd.DataFrame,
                             fiscal_start_date: str) -> pd.DataFrame:
...
@st.cache_data(ttl=CacheConfig.PROCESSED_CACHE_TTL)
def calculate_portfolio_allocation(holdings: pd.DataFrame, stock_prices: pd.DataFrame,
                                   cash_balance: float) -> pd.DataFrame:
    ...
    # Check if we're using shares or shares_held (for compatibility during transition)
    shares_col = 'shares' if 'shares' in allocation.columns else 'shares_held'

    # Always calculate market_value using current price (ignore outdated DB value)
...
class CachedDatabaseInterface:
    ...
    def create_processor(self, start_date: date, end_date: date, fund: str = "DADCO") -> Optional[DataProcessor]:
        ...
        try:
            # Use the new combined returns function
            ...
            return DataProcessor(
                ...
                fund=fund,  # Changed from fund_id to fund
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'next_db_update': st.session_state[CacheConfig.LAST_DB_UPDATE] + timedelta(
                seconds=CacheConfig.DB_CACHE_TTL),
            'next_processed_update': st.session_state[CacheConfig.LAST_PROCESSED_UPDATE] + timedelta(
                seconds=CacheConfig.PROCESSED_CACHE_TTL)
        }
